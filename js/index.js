...
function createColumns(project_items_list) {
	...
	divided_project_section.push(project_items_list.slice(0, group_count).join(""));
	divided_project_section.push(`</div>`);
	divided_project_section.push(`<div class='col-md-4 animate-box' data-animate-effect='fadeInLeft'>`);
	divided_project_section.push(project_items_list.slice(group_count, group_count*2).join(""));
	divided_project_section.push(`</div>`);
	divided_project_section.push(`<div class='col-md-4 animate-box' data-animate-effect='fadeInLeft'>`);
	divided_project_section.push(project_items_list.slice(group_count*2, project_items_list.length).join(""));
	divided_project_section.push(`</div>`);

	return divided_project_section.join("");
}

/* God Bless for ChatGPT converting all my shit into JSON for me ~chefs kiss~ */ 
...
project_showcase_json.forEach(function(item) {
	var item_html = [];
